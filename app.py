...
def user_file(uid:int): return os.path.join(USER_DIR, f"user_{uid}.json")
...
def get_user_id_name(sid:str, base:str):
    info=requests.get(base,
        params={"svc":"core/search_items",
                "sid":sid,
                "params":json.dumps({
...
def schedule_nightly(base,uid,ucfg,gcfg):
    ...
    if uid in timers and timers[uid].is_alive(): timers[uid].cancel()
    ...
    add_script_run_ctx(t); t.start(); timers[uid]=t
...
def main():
    ...
    # init session
    default_tok = qs.get("token",[gcfg.get("last_token","")])[0]
    st.session_state.setdefault("token", default_tok)
    st.session_state.setdefault("sid",   qs.get("sid",[None])[0])

    # resolve user id if sid exists
    ...
    if page=="Admin":
        ...
        if gcfg["admin_pw_hash"]:
            ...
            if st.sidebar.button("Postavi"):
                ...
                else: st.sidebar.error("Lozinka ne može biti prazна.")
            st.stop()
        ...
        st.session_state["token"]=st.text_input("Token", st.session_state["token"],
                                                type="password")
        # ako se token promenio treba novo logovanje
        user_id=None
        if st.session_state.get("sid") and st.session_state.get("user_id"):
            user_id=st.session_state["user_id"]

        tok=st.session_state["token"]
        if not user_id and tok:
            out=login_token(tok, base_url)
            if out:
        ...
        if col1.button("Sačuvaj"):
            ...
            save_global(gcfg); save_user(user_id, ucfg)
    ...
    if "user_id" not in st.session_state:
        st.info("Prijavi se tokenом u Admin panelu."); st.stop()
    user_id=st.session_state["user_id"]
    ucfg=load_user(user_id)
    ucfg.setdefault("recipients","")
    ucfg.setdefault("auto_send",False)
