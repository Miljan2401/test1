...
def main():
    ...
    # dva loga
    logo_a = os.path.join(os.path.dirname(__file__), "app_icon.png")
    logo_b = os.path.join(os.path.dirname(__file__), "app_icon1.png")
    col1, col2 = st.columns([1, 1])
    with col1:
        if os.path.exists(logo_a):
            st.image(logo_a, width=220)
    with col2:
        if os.path.exists(logo_b):
            st.image(logo_b, width=220)
    ...
    if page == "Admin":
        ...
        s["token"] = st.text_input("Token",
                                   s.get("token", DEFAULT_TOKEN),
                                   type="password")
    ...
    # ───────── FILES ─────────
        ...
        with col_left:
            ...
            sel_lbl = st.radio("Lista vozila",
                               ...
                               index=0)
            unit = next(u for u in flt
                        if f"{u['reg']}  —  {u['name']}" == sel_lbl)
