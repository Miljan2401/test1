...
def save_user(h, cfg):
    json.dump(cfg, open(os.path.join(USER_DIR, f"{h}.json"), "w", encoding="utf-8"))
    ...
    if page == "Admin":
        ...
        for fld in ("server", "port", "username", "password"):
            gcfg[fld] = st.text_input(
                fld.capitalize(), gcfg[fld], type="password" if fld == "password" else "default"
            )
        ...
        if col1.button("Sačuvaj"):
            gcfg["last_token"] = st.session_state["token"]   # <— snimi poslednji token
            save_global(gcfg)
            save_user(tok_hash, ucfg)
    ...
    with r:
        ...
        with c1:
            if len(picked) == 1:
                ...
                if data:
                    st.download_button(
                        "Preuzmi fajl", data, picked[0], mime="application/octet-stream"
                    )
            else:
                buf = BytesIO()
                with zipfile.ZipFile(buf, "w") as z:
