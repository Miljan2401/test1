...
def normalize(u:str): return u.rstrip("/") + (
    "/wialon/ajax.html" if not u.rstrip("/").endswith("/wialon/ajax.html") else "")

# ─── global settings (smtp + admin pw) ───────────────────────
def load_global():
...
# ─── per-token settings (recipients + auto_send) ─────────────
def load_user(tok_hash):
...
def save_user(tok_hash,cfg):
    fn=os.path.join(USER_DIR,f"{tok_hash}.json")
    json.dump(cfg,open(fn,"w",encoding="utf-8"))

# ─── Wialon API helperi (isti kao ranije) ────────────────────
...
# ─── mail + scheduler ─────────────────────────────────────────
def send_mail(subj,body,att,fname,global_cfg,recipients):
    try:
        msg=EmailMessage(); msg["Subject"]=subj
        msg["From"]=global_cfg["username"]; msg["To"]=recipients
        msg.set_content(body)
        if att: msg.add_attachment(att,maintype="application",
                                   subtype="zip",filename=fname)
        with smtplib.SMTP(global_cfg["server"],int(global_cfg["port"])) as s:
            s.starttls(); s.login(global_cfg["username"],global_cfg["password"]); s.send_message(msg)
    except Exception as e: st.error(f"SMTP greška: {e}")

def schedule_nightly(base,tok_hash,user_cfg,global_cfg):
    timers=st.session_state.setdefault(TIMERKEY,{})
    if tok_hash in timers and timers[tok_hash].is_alive():
        timers[tok_hash].cancel()
    if not user_cfg.get("auto_send"): return

    now=datetime.now(EU_BG)
    first_run=datetime.combine(now.date()+ (timedelta(days=1) if now.time()>=t(2,5) else timedelta()),
                                t(2,5),tzinfo=EU_BG)
    ...
    def job():
        try:
            ...
            send_mail(f"DDD fajlovi {prev:%d.%m.%Y}",
                      ...
                      buf.read(),f"DDD_{prev}.zip",global_cfg,user_cfg["recipients"])
        finally:
            schedule_nightly(base,tok_hash,user_cfg,global_cfg)
...
def main():
    ...
    if page=="Admin":
        ...
        # ---- GLOBAL SMTP ----
        st.subheader("SMTP (globalno za sve naloge)")
        ...
        # ---- PER-TOKEN POLJA ----
        st.subheader("Podešavanja za ovaj token")
    ...
    with l:
        ...
        lbl=st.radio("Lista vozila",[f"{u['reg']} — {u['name']}" for u in flt],index=0)
        unit=next(u for u in flt if f"{u['reg']} — {u['name']}"==lbl)
        ...
        with c2:
            if st.button("Pošalji e-mail"):
                ...
                send_mail(f"DDD fajlovi — {unit['reg'] or unit['name']}",
                          ...
                          att,fname,gcfg,ucfg["recipients"])
