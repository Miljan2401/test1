...
# ──────────────── POMOĆNE ───────────────────
normalize_base_url = lambda u: u.rstrip("/")+("/wialon/ajax.html"
...
def main():
    ...
    if os.path.exists(logo):
        ...
        st.warning("app_icon.png nije pronađen u repo-u.")
    ...
    # ─────── FILES ───────
        ...
        try: units=get_units(s["sid"],s["base_url"])
        ...
        with col_right:
            ...
            try: files=list_files(s["sid"],unit["id"],day,s["base_url"])
            except Exception as e: st.error(e); st.stop()
