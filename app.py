...
GLOBAL_FILE   = "global_settings.json"   # SMTP + admin pass
USER_DIR      = "user_settings"          # per-token JSONs
TIMERKEY      = "timers"                 # timers in session_state
...
def main():
    ...
    if page == "Admin":
        ...
        tok_hash = sha(st.session_state["token"])      # NOV hash
        ...
        if col1.button("Sačuvaj"):
            save_global(gcfg)
            save_user(tok_hash, ucfg)                 # novi hash!
