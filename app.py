...
GLOBAL_FILE   = "global_settings.json"      # SMTP + admin_pw
USER_DIR      = "user_settings"             # per-token fajlovi
...
def load_global():
    ...
    return {"server":"","port":"587","username":"",
            "password":"","admin_pw_hash":""}

def save_global(c): json.dump(c,open(GLOBAL_FILE,"w",encoding="utf-8"))
...
# ───── Wialon API helperi (skraćeno, ničim promenjeni) ───────
...
def wialon_call(svc,sid,params,base,*,get=False,retry=True):
    ...
    resp=req(base,params=payload if get else None,data=payload if not get else None,timeout=20).json()
...
def list_files(sid,uid,day,base):
    res=wialon_call("file/list",sid,
        {"itemId":uid,"storageType":2,"path":"tachograph/","mask":"*","recursive":False,"fullPath":False},base)
    ...
    for f in res:
        for k in ("ct","mt"):
            ...
            if m and datetime.strptime(m.group(),"%Y%m%d").date()==day: out.append(f)
    out.sort(key=lambda x:x.get("mt",x.get("ct",0)),reverse=True)
    return out

def get_file(sid,uid,f,base):
    r=requests.get(base,params={"svc":"file/get","sid":sid,
        "params":json.dumps({"itemId":uid,"storageType":2,"path":f"tachograph/{f}"})},timeout=20)
    return r.content if r.status_code==200 else None

# ───── mail & scheduler (skraćeno, isto kao pre) ──────────────
def send_mail(subj,body,att,fname,gcfg,rcpt):
    try:
        msg=EmailMessage(); msg["Subject"]=subj; msg["From"]=gcfg["username"]; msg["To"]=rcpt
        msg.set_content(body)
        if att: msg.add_attachment(att,maintype="application",subtype="zip",filename=fname)
...
def schedule_nightly(base, tok_hash, ucfg, gcfg):
    ...
    def job():
        ...
        finally: schedule_nightly(base,h,ucfg,gcfg)
...
def main():
    ...
    gcfg=load_global()   # SMTP + admin_pw
        ...
        if gcfg["admin_pw_hash"]:
            if not st.session_state.get("admin_ok"):
                ...
                if st.sidebar.button("Login"):
                    st.session_state["admin_ok"]=sha(pwd)==gcfg["admin_pw_hash"]
    ...
    if not st.session_state["sid"]:
        if st.button("Login tokenom"):
            ...
            if sid:
                st.session_state["sid"]=sid; schedule_nightly(base_url,tok_hash,ucfg,gcfg)
                st.experimental_rerun()
        st.info("Prijavi se tokenom."); st.stop()

    try: units=get_units(st.session_state["sid"],base_url)
    ...
    with r:
        ...
        with c1:
            if len(picked)==1:
                ...
                if data: st.download_button("Preuzmi fajl",data,picked[0],mime="application/octet-stream")
            else:
                buf=BytesIO()
                with zipfile.ZipFile(buf,"w") as z:
                    for fn in picked:
        ...
        with c2:
            if st.button("Pošalji e-mail"):
                if len(picked)==1:
                    att=get_file(st.session_state["sid"],unit["id"],picked[0],base_url); fname=picked[0]
                else:
                    buf=BytesIO()
                    with zipfile.ZipFile(buf,"w") as z:
                        for fn in picked:
                ...
                send_mail(f"DDD fajlovi — {unit['reg'] or unit['name']}",
                          "Izabrani fajlovi u prilogu.",att,fname,gcfg,ucfg["recipients"])
